// 出处：https://github.com/mofaph/csapp/blob/master/exercise/ex2-95.c
 ...
 *
 * 当一个整数需要至少 24 位表示，则表明这个整数转换成浮点数时需要舍入。
 * 由于一个整数转换将会转换成规格化的浮点数，因此最左边的 1 将会成为隐藏的
 * 小数部分。（0 是唯一的非规格化数）
 *
 * 当转换的数字是负数时，由于补码表示的原因，因此我们需要把它转换成正数表示
 * 的数字形式。同时，我们需要记录负数的符号位。
 *

 ...
 * 
 * 1. 添加回收后台子进程的功能
 * 2. 添加一系列函数，处理作业组，init_jobs(), add_job(pid), delete_job(pid)
 * 3. 添加一个打印所有后台作业的函数，print_jobs();
 * 4. 添加一个给所有前台进程组中的每个进程发送信号的函数
 */
...
int builtin_command(char **argv)
{
	...
	else (!strcmp(argv[0], "&")) 	  /* Ignore singleton & */
		return 1;	
